    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-16">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-bold text-text-primary mb-3">Core Services</h2>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {services.map((service) => (
            <Link
              key={service.title}
              href={service.href}
              className="group relative overflow-hidden rounded-2xl border border-subtle bg-dark-800 p-6 hover:border-accent-coral/30 hover:shadow-glow-coral transition-all duration-300"
            >
              ...
              <div className="relative">
                ...
                <h3 className="text-xl font-bold text-text-primary mb-2 group-hover:text-accent-coral transition-colors">
                  {service.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {service.features.map((feature) => (
                    <span
                      key={feature}
                      className="px-2 py-1 bg-dark-700 text-text-muted text-xs rounded-md border border-subtle"
                    >
                      {feature}
                    </span>
                  ))}
                </div>
                <div className="mt-4 text-sm font-medium text-accent-coral flex items-center gap-1 group-hover:gap-2 transition-all">
                  Learn more
      ...
      {/* API Preview */}
      <div className="mb-16 rounded-2xl border border-subtle bg-dark-800 p-8 md:p-10 overflow-hidden">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            <h2 className="text-2xl font-bold text-text-primary mb-4">Simple, Powerful API</h2>
            ...
            <Link
              href="/docs/api"
              className="inline-flex items-center gap-2 text-accent-coral font-medium hover:text-accent-coral-hover transition-colors"
            >
              Explore the API
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          ...
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-r from-accent-coral to-accent-cyan rounded-xl blur-xl opacity-10" />
            <div className="relative bg-dark-900 rounded-xl p-4 font-mono text-sm overflow-x-auto border border-subtle">
...
console.log(claim.evidence_count); // 23`}
      ...
      <div className="mb-16">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-bold text-text-primary mb-3">What You Can Build</h2>
          <p className="text-text-secondary max-w-2xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {useCases.map((useCase) => (
            <div
              key={useCase.title}
              className="p-6 bg-dark-800 rounded-xl border border-subtle hover:border-subtle-hover transition-colors"
            >
      ...
      {/* Rate Limits & Tiers */}
      <div className="mb-16 rounded-2xl border border-subtle bg-dark-800 p-8 md:p-10">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-text-primary mb-3">Generous Rate Limits</h2>
          ...
          {[
            ...
            <div
              ...
              className={`p-6 rounded-xl border-2 ${
                tier.color === 'default' ? 'border-subtle bg-dark-700' :
                tier.color === 'coral' ? 'border-accent-coral/30 bg-accent-coral/5' :
                'border-accent-cyan/30 bg-accent-cyan/5'
              }`}
            >
              <div className={`text-sm font-bold uppercase tracking-wide mb-2 ${
                tier.color === 'default' ? 'text-text-muted' :
                tier.color === 'coral' ? 'text-accent-coral' :
                'text-accent-cyan'
              }`}>
                {tier.tier}
              </div>
              <div className="text-3xl font-bold text-text-primary mb-1">{tier.requests}</div>
              <div className="text-sm text-text-muted mb-4">{tier.period}</div>
      ...
      {/* Bottom CTA */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-accent-coral to-accent-coral-hover p-8 md:p-12 text-center">
        ...
        <div className="relative">
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            <Link
              href="/docs/api"
              className="inline-flex items-center gap-2 px-6 py-3 bg-white text-accent-coral rounded-xl font-semibold hover:bg-gray-100 transition-all shadow-lg"
            >
              Read the Docs
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
